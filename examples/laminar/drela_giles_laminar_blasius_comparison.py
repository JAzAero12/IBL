...
def compare_blasius_solution() -> None:
    ...
    colors = ["green", "red"]
    color = "green"
    labels = ["Blasius", "Drela-Giles, Laminar"]
    ...
    #                                  d2U_edx2=d2u_e_fun, data_fits="Spline")]
    model = DrelaGilesLaminar(nu = nu_inf, U_e= u_e_fun, dU_edx= du_e_fun, ic = ManualCondition(delta_d=delta_d_exact[0], delta_m=delta_m_exact[0],delta_k=0))
    ...
    re_del_m_exact = u_inf*delta_m_exact/nu_inf
    re_del_m = u_inf*delta_m/nu_inf
    c_f_Q = model._c_f_dg(shape_d,re_del_m)
    #c_f_Q = model._c_f_dg(shape_d_exact,re_del_m_exact)
    #c_f = c_f_Q
    v_e = model.v_e(x)
    print(abs(delta_m-delta_m_exact))
