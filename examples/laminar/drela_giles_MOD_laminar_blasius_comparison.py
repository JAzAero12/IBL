...
def compare_blasius_solution() -> None:
    ...
    colors = ["#A4D65E","#154734"]
    #color = "green"
    labels = ["Blasius", "D-G, Old", "D-G, Modern"]
    ...
    _ = axis_delta_d[1].plot(x/c, np.abs(1-delta_d/delta_d_exact),
                             color=colors[0])
    _ = axis_delta_m[0].plot(x/c, delta_m/c, color="#154734")
    _ = axis_delta_m[1].plot(x/c, np.abs(1-delta_m/delta_m_exact),
                             color="#154734")
    _ = axis_shape_d[0].plot(x/c, shape_d, color=colors[0])
    _ = axis_shape_d[1].plot(x/c, np.abs(1-shape_d/shape_d_exact),
                             color=colors[0])
    _ = axis_c_f[0].plot(x/c, c_f, color="#F8E08E")
    _ = axis_c_f[1].plot(x/c, np.abs(1-c_f/c_f_exact), color="#F8E08E")
    ...
    _ = axis_delta_m[0].plot(x/c, delta_m_src/c, color="#154734",linestyle='--')
    _ = axis_delta_m[1].plot(x/c, np.abs(1-delta_m_src/delta_m_exact),
                             color="#154734",linestyle='--')
    _ = axis_shape_d[0].plot(x/c, shape_d_src, color=colors[0],linestyle='--')
    _ = axis_shape_d[1].plot(x/c, np.abs(1-shape_d_src/shape_d_exact),
                             color=colors[0],linestyle='--')
    _ = axis_c_f[0].plot(x/c, c_f_src, color="#F8E08E",linestyle='--')
    _ = axis_c_f[1].plot(x/c, np.abs(1-c_f_src/c_f_exact), color="#F8E08E",linestyle='--')
    ...
    plt.show()
    #TODO do in unittest, do Falkner-Skan
